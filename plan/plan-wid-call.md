...
### 2b. Боевой тест, раунд 2 — лаунчер/embed (коммит `aa1aa6c`)

| # | Симптом | Root cause | Фикс |
|---|---------|-----------|------|
| 4 | Положение виджета **не учитывалось** (всегда справа) | `Callback/embedded/embedRuntime.tsx` хардкодил `fixed bottom-6 right-6`, не читая `launcher.position`. | Враппер ставит лаунчер по `position` — `left-6` / `left-1/2 -translate-x-1/2` / `right-6`. |
| 5 | У **бабл-уведомления нет «закрыть»** | В `CallbackLauncher` бабл не имел кнопки. | Добавлен крестик (× в углу бабла): закрывает уведомление. |
| 6 | Бабл **снова всплывает через время** после закрытия | `useEffect` перепланировал бабл при каждом тогле `open`; не было флага «закрыто пользователем». | Флаг `dismissed`: после закрытия бабла ИЛИ открытия виджета бабл больше не всплывает автоматически. |
| 7 | Бабл/ореол **обрезаются** (особенно после открыть→закрыть) | Клип считался по `cbRoot.getBoundingClientRect()` — не учитывает абсолютно-позиционированный пульс-ореол (`transform: scale`) и тени; запас 36px мал. | Клип = `mergeRects(root + все потомки)` (ловит ореол/бабл/крестик) + запас **44px**. |
| 8 | **Дублирование виджета** (два лаунчера) | Нет защиты от двойного монтирования при двойном подключении/инициализации `embed.js`. | Глобальный per-widget mount-реестр `window.__lemnityMounted`: второй экземпляр менеджера на тот же `widgetId` не монтирует второй виджет. **Точная причина на сайте пользователя не подтверждена** — диагностика: `document.querySelectorAll('[data-lemnity-embed-container]').length` (ожидаем 1; если сниппет на странице дважды — убрать дубль). |
