...
- **Асистент** (`AssistantSection`) — управление ИИ-агентом (реализовано 2026-06-16, см. секцию
  «ИИ-агент чата» ниже): селектор чата, **тумблер включения** агента, карточка **статистики остатка**
  (месяц + сегодня), кнопка **«Настроить ИИ-агента»** (раскрывает блок «Аи агент»: имя + страницы),
  **индикатор статуса** (Активен / Выключен / Не подключён / Лимит).
...
| **Асистент** (тумблер ИИ + статистика остатка + настройки) | РЕАЛЬНЫЙ (`AiAgentService`, `/chat/ai-usage`) | есть |
...
- ~~Раздел **Асистент** (настройки ИИ)~~ — **реализовано 2026-06-16** (см. секцию «ИИ-агент чата» ниже).
...
## ИИ-агент чата (2026-06-16, commits `f3aaadc`, `ff3fe2f`, `8eab1ba`, `a651a29`)

Полноценный ИИ-агент: отвечает посетителям **на их языке** на основе **сайта/продукта клиента**,
под именем из настроек. Провайдер — **OpenRouter** (бесплатный DeepSeek больше недоступен → дефолт
`openai/gpt-oss-120b:free`, модель в env). Поведение: при включённом агенте отвечает на все сообщения
посетителя (free-text) — серверный `maybeReply` срабатывает на `message:send` визитёра.

**Бэкенд** (`projects/server/src/chat/`):
- `AiAgentService` — `maybeReply(conversationId, onTyping?)`: гейт по `aiAgentEnabled` и `channel='web'`,
  проверка квоты, сборка промпта (персона+язык+сайт), вызов OpenRouter, ответ как `sender='manager'` +
  `aiGenerated=true`. Анти-дабл-ответ (in-flight set), best-effort. `getUsage(widgetId)` — месячная и
  дневная квота; `discoverPages(widgetId)` — внутренние страницы сайта.
- `SiteKnowledgeService` — MVP-краул `Project.websiteUrl` (главная + выбранные страницы), извлечение
  текста (без cheerio), кэш в таблице **`AiSiteKnowledge`** (per-project, TTL 7 дней, сигнатура по набору
  страниц). **SSRF-защита**: http/https, DNS-резолв с блоком приватных IP, ручная проверка редиректов,
  ограничение по хосту сайта.
- Эндпоинты в `chat.controller.ts` (под `ChatActorGuard`, scope-проверка `assertActorOwnsWidget`):
  `GET /chat/ai-usage?widgetId=` → `{configured, limit/used/remaining/resetAt, dailyLimit/…}`;
  `GET /chat/ai-pages?widgetId=` → список внутренних страниц для пикера.
- Prisma: `ChatMessage.aiGenerated` (флаг ответа ИИ; индекс для подсчёта квоты) + модель
  `AiSiteKnowledge`. Применяется через `db push`.
- env: `OPENROUTER_API_KEY` (секрет, в прод-`.env` на сервере), `AI_AGENT_MODEL`,
  `AI_AGENT_DAILY_LIMIT=40`, `AI_AGENT_MONTHLY_LIMIT=1000`, `AI_AGENT_SITE_URL/NAME` (HTTP-Referer).
  Проброс в контейнер — `docker-compose.prod.yml` (с дефолтами). На проде ключ задан 2026-06-16.

**Лимиты**: на чат-виджет — **1000/мес** + **40/день** (дневной — реальный потолок free-tier OpenRouter
~50/день на аккаунт). При исчерпании любого — ИИ молчит, отвечает оператор. Счётчик — `aiGenerated`-сообщения
за период (UTC).

**Пометка «ИИ» — только оператору**: `aiGenerated` отдаётся в операторском REST/у бейджа в кабинете;
визиторский REST (`toMessageEntityForVisitor`) флаг скрывает. Посетитель видит ответ под именем агента,
без бейджа.

**Раздел «Ассистент»** (`AssistantSection` в `ChatModulePage`): тумблер `aiAgentEnabled`, карточка остатка
(месяц + «сегодня X/40»), кнопка «Настроить ИИ-агента» (раскрывает `ChatAiAgentSettings`), индикатор
статуса (Активен/Выключен/Не подключён/Лимит) по `configured`+квотам.

**Блок «Аи агент»** (`ChatAiAgentSettings`, виджет-настройки): имя агента + **«Разделы для изучения»** —
пикер внутренних страниц сайта (`GET /chat/ai-pages`); пусто → агент изучает только главную (лендинг).

**Сценарий — шаг «Аи агент»** (`ScenarioEditor`): кнопка «+ Шаг» → меню (💬 Шаг / 🤖 Аи агент). AI-узел —
`ScenarioStep.agent`, точки связи с двух сторон. В рантайме виджета (`embedRuntime`) при достижении AI-шага
диалог переключается в free-text → отвечает серверный ИИ.

**Имитация живого человека**: статус «печатает…» во время генерации (`onTyping` → `operator:typing` в
комнату диалога), «человеческая» пауза набора (~12мс/символ, 0.5–2.5с); плавная анимация появления пузырей
(`framer-motion`, fade+сдвиг+масштаб) — `Widget.tsx`.

## Батч UX-правок кабинета (2026-06-16, commits `5b27244`, `fda9c8f`, `a64867e`)

- **Панель форматирования оператора** (`ChatModulePage`, поле ввода): тулбар B / I / ~~S~~ / ссылка /
  маркированный / нумерованный список — вставка markdown по выделению (`applyWrap`/`applyLinePrefix`/
  `insertLink`, ref на textarea). Рендер markdown в пузырях кабинета и виджета — общий `common/markdownLite.tsx`
  (безопасно, без `dangerouslySetInnerHTML`; ссылки только http(s)/mailto). **Enter** — перенос строки,
  отправка только кнопкой «Отправить».
- **Блок «Информация» сворачивается в сайдбар** (правая панель диалога): при сворачивании — узкая полоска
  с кнопкой разворота; заметная кнопка-шеврон (как у сайдбара модуля) + иконка.
- **Чекбоксы в списке диалогов + «Убрать в архив»**: сбоку каждой строки инбокса чекбокс (вне кнопки-строки),
  панель массового действия «Убрать в архив (N)» → `chatsService.updateConversationStatus(id,'closed')` →
  диалоги уходят из «Все диалоги» в «Завершённые». Фильтр `filtered` уже разделяет по `status`.
- **Соцсети компании — блок настроек** (`ChatSocialLinksSettings`, секция `chat.company-socials` под
  «Контакты компании»): список соцсетей (иконки/пресеты из «Мультикнопки» FAB_MENU на бренд-подложке) +
  редактор заголовка вкладки (текст/выравнивание/размер/цвет/толщина — как «Приветственный заголовок»).
  См. виджет-сторону в `plan-wid-chat.md`. Поля схемы `companySocials`, `socialsTitle*` (optional).
- **«Бот устал»** (серверный fallback, `chat.gateway` + `AiAgentService.maybeReply` → status): если агент
