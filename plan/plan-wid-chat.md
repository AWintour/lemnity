...
- [x] **Бот-сценарий (редактор чат-бота)** — модель «дерево быстрых ответов»:
      ...
      **Изображение в шаге**: `scenario.steps[].image?` (data-URL/URL) — в узле редактора кнопка
      «🖼️ Добавить изображение» (FileReader → data-URL) + превью с удалением; в окне чата картинка
      рендерится в бабббле бота над текстом (шаг показывается даже если только картинка). ⚠️ Сейчас
      картинка хранится data-URL'ом прямо в конфиге — для прод нужен аплоад файла в хранилище + URL.
...
- [ ] Аплоад изображений шага сценария в хранилище (сейчас data-URL в конфиге).
