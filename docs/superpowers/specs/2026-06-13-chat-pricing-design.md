...
> подписка как источник истины. В этом монорепо (server+client) — **только энфорсмент**: гейт
> обслуживания по оплате и носитель лимита менеджеров. Зеркало паттерна Callback
> (см. [plan-wid-call.md](../../../plan/plan-wid-call.md), раздел «Монетизация»).
...
### 3.1. Гейт обслуживания по оплате (закрыть текущую дыру) — СЕЙЧАС
...
- Не вводим модель-B «пакеты тарифов» (Базовый/Pro/Корп) — выбрана flat-база + доп. менеджер.
