/* =========================================================
   Lemnity — партнёрская программа. Интерактив.
   ========================================================= */
(function () {
  "use strict";

  // --- TODO: подставить боевые URL перед публикацией ---
  var LINKS = {
    register: "https://lemnity.ru/login",
    partner: "https://lemnity.ru/login", // вход/регистрация партнёра
  ...
  // Источник истины по тарифам — виджет «Чат» (lmntai): Старт 0 · Старт+ 590 · Про 1 490 · Бизнес 3 490.
  // Расчёт партнёрской маржи берём от тарифа «Про» = 1 490 ₽/мес за сайт.
  // Ступени комиссии: 1–2 сайта — 7%, 3–9 — 10%, 10+ — 15%. Энтерпрайз — договорной (вне калькулятора).
  var BASE = 1490;
  var slider = document.getElementById("calc-range");
  if (slider) {
    var elSites = document.getElementById("calc-sites");
    var elYear = document.getElementById("calc-year");  // годовой — крупное число
    var elMonth = document.getElementById("calc-month"); // помесячный — строкой
    var elTier = document.getElementById("calc-tier");
    var elRate = document.getElementById("calc-rate");
    ...
    var tierFor = function (sites) {
      if (sites >= 10) return { name: "Золото", rate: 0.15 };
      if (sites >= 3) return { name: "Серебро", rate: 0.10 };
      return { name: "Партнёр", rate: 0.07 };
    };
    var render = function () {
      var sites = parseInt(slider.value, 10);
      var t = tierFor(sites);
      var month = sites * BASE * t.rate;
      elSites.textContent = sites + " " + plural(sites, ["сайт", "сайта", "сайтов"]);
      elYear.innerHTML = fmt(month * 12) + " <span>₽/год</span>"; // годовой — главный якорь
      elMonth.textContent = fmt(month) + " ₽";
      elTier.textContent = t.name;
      elRate.textContent = Math.round(t.rate * 100) + "%";
    };
    slider.addEventListener("input", render);
    render();
  }

  // --- Анимация чата в мокапе ---
