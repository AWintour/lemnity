...
export class AiAgentService {
  ...
  async maybeReply(
    ...
    try {
      ...
      // Гарантированно убираем подпись именем, даже если модель её добавила вопреки промпту.
      answer = this.stripSignature(answer, cfg.name)
      if (!answer) return unavailable
  ...
  /**
   * Срезает хвостовую подпись именем агента: «… — Лемнити )», «…\n– Лемми :)» и т.п.
   * Детерминированная страховка — модель иногда подписывается вопреки промпту. Точный матч
   * по имени, только в конце текста — легитимные тире/имя в середине не трогаем.
   */
  private stripSignature(text: string, name: string): string {
    if (!text || !name) return text
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    const re = new RegExp(`\\s*[—–\\-]\\s*${escaped}\\s*[)\\s:;.!(]*$`, 'iu')
  ...
  private buildSystemPrompt(name: string, site: string): string {
    ...
    return [
      ...
      '2. Отвечай по сути ТОЛЬКО на основе информации о продукте и сайте клиента из блока ниже. Не придумывай факты, услуги, вакансии, цены, списки и детали, которых там нет.',
      ...
      '5. Не предлагай оставить телефон или e-mail по своей инициативе и не делай это дежурной концовкой ответа. Проси контакты ТОЛЬКО если пользователь сам попросил связать его с человеком/менеджером или явно на это согласился — и не более одного раза за диалог.',
